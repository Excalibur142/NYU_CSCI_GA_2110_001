...
case class Leaf[T](x : T) extends Tree[T] { // Roughly 4 lines
  val num: T = x
  override def toString() = (s"Leaf($num)")
  override def map[E >: T](f: T => E): Tree[E] = Leaf[E](f(num))
}
...
case class Node[T](x : T, children : List[Tree[T]]) extends Tree[T] { // Roughly 4 lines
  val num: T = x
  val child: List[Tree[T]] = children
  override def toString(): String = "Node("+ x + ","+child+ ")"
  override def map[E >: T](f: T => E): Tree[E] = Node[E](f(num), child.map(fn => fn.map(f)))
}
...
class B(x:Int, y:Int) extends A(x){ // Roughly 4 lines
  val numY:Int = y
  override def value: Int = this.x + this.numY
  override def toString(): String = ("B("+num+","+numY+")")
}
...
object Part2 {
  ...
  def breadthFirst[T](tree : Tree[T]) : List[T] = { // Roughly 9 lines
    def breadth[T](treeList: List[Tree[T]], l: List[T]): List[T] = treeList match {
      case Nil => l
      case (Leaf(x) :: rest) => breadth(rest, l ++ List[T](x))
      case (Node(x, children) :: rest) => breadth(rest ++ children, l ++ List[T](x))
      case _ => List()
    }
    breadth(List(tree), List[T]())
  }
  //  breadth(tree, List[T]())
  //}
  ...
  def reduce[T, E](f:(T,E) => E, b : E, l : List[T]) : E = { // Roughly 4 lines
    l match {
      case Nil => b
  ...
  def sumTreeA(t: Tree[A]):A  = { // Roughly 4 lines
    t match {
      case Leaf(x) => x
      case Node(x, children) => reduce((a:A, b:A) => a +b, x, children.map(sumTreeA))
    }
  }
